...
scrapper.views = (function() {
  ...
  function cancel(e) {
    ...
    id = this.$table.fnGetData()[position[0]]._id;
    if (id) {
      setEditorDisplay($(e.currentTarget).parents('tr'),false);
    } else {
  ...
  views.UserListView = {
	...
	.dataTable({
	  ...
	  "aoColumns" : [
		...
		if (full._id) {
		  return '<a class="delete readOnly" data-id="'
		    + full._id
		    + '">x</a> <a class="edit readOnly">Edit</a> <a class="save editor" style="display:none">Save</a><a class="cancel editor" style="display:none">CANCEL</a>';
		} else {
		  return '<a class="cancel">Cancel</a><a class="save">Save</a>';
		}
	      }
	    }, {
	      "mData" : "userId",
